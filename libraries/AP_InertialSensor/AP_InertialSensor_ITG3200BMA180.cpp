...
#define BMA180_ACCELEROMETER_TCO_Z        0x30
#define BMA180_ACCELEROMETER_DATA         0X02
#define BMA180_ACCELEROMETER_GRAVITY      255

// BMA180 accelerometer scaling
// Result will be scaled to 1m/s/s
#define BMA180_ACCELEROMETER_SCALE_M_S    (GRAVITY_MSS / 4096.0f)
...
bool AP_InertialSensor_ITG3200BMA180::_init_sensor(void)
{
    ...
    if (data != BMA180_ACCELEROMETER_CHIPID)
        AP_HAL::panic("AP_InertialSensor_ITG3200_BMA180: could not find BMA180 accelerometer sensor");

    // default range 2G: 1G = 4096 unit.
    // register: ctrl_reg0  -- value: set bit ee_w to 1 to enable writing
    hal.scheduler->delay(5);

    hal.i2c->writeRegister(BMA180_ACCELEROMETER_ADDRESS, BMA180_ACCELEROMETER_PWR, 1<<4);

    hal.scheduler->delay(5);

    hal.i2c->readRegister(BMA180_ACCELEROMETER_ADDRESS, BMA180_ACCELEROMETER_BW, &control);
    control = control & 0x0F;        // save tcs register
    control = control | (0x01 << 4); // set low pass filter to 10Hz (bits value = 0000xxxx)

    hal.i2c->writeRegister(BMA180_ACCELEROMETER_ADDRESS, BMA180_ACCELEROMETER_BW, control);

    hal.scheduler->delay(5);

    hal.i2c->readRegister(BMA180_ACCELEROMETER_ADDRESS, BMA180_ACCELEROMETER_TCO_Z, &control);
    control = control & 0xFC;        // save tco_z register
    control = control | 0x00;        // set mode_config to 0
    hal.i2c->writeRegister(BMA180_ACCELEROMETER_ADDRESS, BMA180_ACCELEROMETER_TCO_Z, control);

    hal.scheduler->delay(5);

    hal.i2c->readRegister(BMA180_ACCELEROMETER_ADDRESS, BMA180_ACCELEROMETER_RANGE, &control);
    control = control & 0xF1;        // save offset_x and smp_skip register
    control = control | (0x05 << 1); // set range to 8G
    hal.i2c->writeRegister(BMA180_ACCELEROMETER_ADDRESS, BMA180_ACCELEROMETER_RANGE, control);
...
//    _product_id = AP_PRODUCT_ID_ITG3200BMA180;
